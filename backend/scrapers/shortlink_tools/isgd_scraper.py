...
def get_isgd_url(payload):
    try:
        ...
        if r.status_code != 200:
            return {"success": False, "error": f"is.gd API returned status code {r.status_code}: {r.text}"}
        
        short_url = r.text.strip()
        if not short_url.startswith("http://") and not short_url.startswith("https://"):
            return {"success": False, "error": f"Invalid response received from is.gd: {short_url}"}
