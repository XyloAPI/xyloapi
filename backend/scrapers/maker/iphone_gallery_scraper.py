...
ASSETS_DIR = os.path.join(os.path.dirname(__file__), "iphone_gallery_assets")
UGUU_URL = "https://uguu.se/upload.php"
IPHONE_GALLERY_TEMPLATE_URL = "https://i.imgur.com/hZIIuWZ.png"

def _upload_to_uguu(binary: bytes) -> str:
    filename = f"iphone_gallery_{int(time.time())}.png"
    res = requests.post(
        UGUU_URL,
...
def generate_iphone_gallery(payload):
    try:
        ...
        template_path = get_cached_file(IPHONE_GALLERY_TEMPLATE_URL, "iphone_gallery_template.png")
        ...
        # Transparent box dimensions from scan: x1=33, y1=144, x2=918, y2=1957
        target_w = 886
        target_h = 1814
        paste_x = 33
        paste_y = 144

        # 3. Crop and resize user image to fill frame
        ...
        return {"success": False, "error": f"Failed to generate iPhone gallery: {str(e)}"}
