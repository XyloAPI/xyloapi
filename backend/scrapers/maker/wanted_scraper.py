...
ASSETS_DIR = os.path.join(os.path.dirname(__file__), "wanted_assets")
UGUU_URL = "https://uguu.se/upload.php"
WANTED_TEMPLATE_URL = "https://i.imgur.com/u2a1qhT.png"

def _upload_to_uguu(binary: bytes) -> str:
    filename = f"wanted_{int(time.time())}.png"
    res = requests.post(
        UGUU_URL,
...
def generate_wanted(payload):
    try:
        ...
        template_path = get_cached_file(WANTED_TEMPLATE_URL, "wanted_template.png")
        ...
        # Transparent box dimensions from scan: x1=25, y1=138, x2=396, y2=503
        target_w = 372
        target_h = 366
        paste_x = 25
        paste_y = 138
        ...
        return {"success": False, "error": f"Failed to generate Wanted poster: {str(e)}"}
