...
ASSETS_DIR = os.path.join(os.path.dirname(__file__), "trumptweets_assets")
UGUU_URL = "https://uguu.se/upload.php"

PORTRAIT_URL = "https://i.imgur.com/9l66omG.jpeg"
...
def _upload_to_uguu(binary: bytes) -> str:
    filename = f"trumptweets_{int(time.time())}.png"
    res = requests.post(
        UGUU_URL,
...
def get_cached_file(url, filename, is_image_and_crop=False):
    ...
    if not os.path.exists(file_path):
        ...
        if is_image_and_crop:
            img = Image.open(io.BytesIO(res.content))
            left = 425
            right = 1825
            top = 250
            bottom = 1650
            pfp = img.crop((left, top, right, bottom))
            pfp = pfp.resize((120, 120), Image.Resampling.LANCZOS)
            pfp.save(file_path, "PNG")
        else:
...
def generate_trumptweets(payload):
    try:
        text = str(payload.get("text") or "MAKE AMERICA GREAT AGAIN!").strip()
        theme = str(payload.get("theme") or "light").strip().lower()
        retweets = str(payload.get("retweets") or "14.5K").strip()
        quotes = str(payload.get("quotes") or "3.2K").strip()
        likes = str(payload.get("likes") or "128K").strip()
        
        now = datetime.datetime.now()
        date_str = str(payload.get("date") or now.strftime("%b %d, %Y")).strip()
        time_str = str(payload.get("time") or now.strftime("%I:%M %p")).strip()
        device_str = str(payload.get("device") or "Twitter for iPhone").strip()

        pfp_path = get_cached_file(PORTRAIT_URL, "trump_pfp.png", is_image_and_crop=True)
        ...
        pfp = Image.open(pfp_path).convert("RGBA").resize((avatar_size, avatar_size), Image.Resampling.LANCZOS)
        mask = Image.new("L", (avatar_size, avatar_size), 0)
        mask_draw = ImageDraw.Draw(mask)
        mask_draw.ellipse((0, 0, avatar_size, avatar_size), fill=255)
        card.paste(pfp, (24, top_padding), mask=mask)

        draw.text((96, 30), "Donald J. Trump", fill=text_color, font=font_name)
        
        name_bbox = draw.textbbox((96, 30), "Donald J. Trump", font=font_name)
        ...
        draw.text((96, 56), "@realDonaldTrump", fill=gray_text_color, font=font_handle)
        ...
        return {"success": False, "error": f"Failed to generate Trump tweet image: {str(e)}"}
