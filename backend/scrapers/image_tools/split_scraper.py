import base64
import requests
import io
import time
import os
from PIL import Image

try:
    from uguu_uploader import upload_uguu
except ImportError:
    import sys
    sys.path.append(os.path.dirname(os.path.abspath(__file__)))
    from uguu_uploader import upload_uguu

def get_split_image(payload):
    image_data = payload.get("image") or payload.get("url") or payload.get("data") or ""
    if not image_data:
        return {"success": False, "error": "Missing required parameter: image or url"}

    rows = payload.get("rows")
    if rows is None:
        rows = 2
    cols = payload.get("cols")
    if cols is None:
        cols = 2

    session = requests.Session()
    session.headers.update({
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    })

    # 1. Fetch image bytes from URL or Base64
    try:
        is_url = image_data.startswith("http://") or image_data.startswith("https://")
        if is_url:
            r_img = session.get(image_data, timeout=15)
            if r_img.status_code != 200:
                return {"success": False, "error": f"Failed to download image from URL. Status code: {r_img.status_code}"}
            img_bytes = r_img.content
        else:
            if "," in image_data:
                image_data = image_data.split(",")[1]
            img_bytes = base64.b64decode(image_data)
    except Exception as e:
        return {"success": False, "error": f"Failed to retrieve image data: {str(e)}"}

    # 2. Split image
    try:
        img = Image.open(io.BytesIO(img_bytes))
        width, height = img.size

        # Constraints: 1 to 10 rows and cols
        rows = max(1, min(10, int(rows)))
        cols = max(1, min(10, int(cols)))

        slice_width = width // cols
        slice_height = height // rows

        slices = []
        for r in range(rows):
            for c in range(cols):
                left = c * slice_width
                top = r * slice_height
                right = (c + 1) * slice_width if c < cols - 1 else width
                bottom = (r + 1) * slice_height if r < rows - 1 else height

                slice_img = img.crop((left, top, right, bottom))
                out_buf = io.BytesIO()
                # Use same format if possible or PNG as default fallback
                fmt = img.format if img.format else "PNG"
                slice_img.save(out_buf, format=fmt)
                slices.append({
                    "row": r,
                    "col": c,
                    "width": right - left,
                    "height": bottom - top,
                    "bytes": out_buf.getvalue(),
                    "format": fmt.lower()
                })
    except Exception as e:
        return {"success": False, "error": f"Failed to split image: {str(e)}"}

    # 3. Upload split slices to Uguu.se
    uploaded_slices = []
    for s in slices:
        try:
            mime = "image/png" if s["format"] == "png" else "image/jpeg"
            b64_str = f"data:{mime};base64," + base64.b64encode(s["bytes"]).decode('utf-8')
            uguu_res = upload_uguu({"image": b64_str})
            if uguu_res.get("success"):
                uguu_url = uguu_res.get("data", {}).get("link")
                uploaded_slices.append({
                    "row": s["row"],
                    "col": s["col"],
                    "width": s["width"],
                    "height": s["height"],
                    "url": uguu_url
                })
        except:
            pass

    if len(uploaded_slices) != len(slices):
        return {"success": False, "error": "Failed to upload all split sections to CDN"}

    # 4. Upload original image to Uguu.se
    uguu_original_url = None
    try:
        original_b64 = "data:image/jpeg;base64," + base64.b64encode(img_bytes).decode('utf-8')
        original_res = upload_uguu({"image": original_b64})
        if original_res.get("success"):
            uguu_original_url = original_res.get("data", {}).get("link")
    except:
        pass

    original_url = uguu_original_url
    if not original_url and is_url:
        original_url = image_data

    return {
        "success": True,
        "retrieved_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "data": {
            "slices": uploaded_slices,
            "original": original_url,
            "rows": rows,
            "cols": cols,
            "width": width,
            "height": height,
        }
    }
