...
def get_blurred_image(payload):
    ...
    try:
        ...
        if x is not None and y is not None and w is not None and h is not None:
            ...
            blurred_crop = cropped.filter(ImageFilter.GaussianBlur(radius))
            
            # Avoid in-place modification side-effects
            out_img = img.copy()
            out_img.paste(blurred_crop, box)
            img = out_img
        else:
        ...
        blurred_bytes = out_buf.getvalue()
    except Exception as e:
        return {"success": False, "error": f"Failed to apply blur to image: {str(e)}"}

    # 3. Upload blurred image to Uguu.se
    uguu_blur_url = None
    try:
        ...
        if uguu_res.get("success"):
            uguu_blur_url = uguu_res.get("data", {}).get("link")
    except:
        pass

    if not uguu_blur_url:
