...
def get_blurface_image(payload):
    ...
    if strength is None:
        strength = 500
    try:
        strength = max(1, min(2000, int(strength)))
    except:
        strength = 500
    ...
    # 3. Construct Face Blur Transformation URL and download the blurred image bytes
    blurred_bytes = None
    transformed_url = f"https://res.cloudinary.com/{cloud_name}/image/upload/e_blur_faces:{strength}/v{version}/{public_id}.{img_format}"
    
    try:
        r_blurred = session.get(transformed_url, timeout=20)
        if r_blurred.status_code == 200:
            blurred_bytes = r_blurred.content
        else:
            # Check if there was an error downloading the transformed image
            # Cloudinary returns headers detailing transformation errors in some cases
            err_msg = r_blurred.headers.get("x-cld-error") or f"Status code: {r_blurred.status_code}"
    ...
    # 4. Clean up / Delete uploaded image from Cloudinary (since we have the processed bytes)
    try:
        destroy_timestamp = int(time.time())
        destroy_params = {"public_id": public_id, "timestamp": str(destroy_timestamp)}
        destroy_sig = generate_cloudinary_signature(destroy_params, api_secret)
    ...
    try:
        ...
        b64_str = f"data:{mime};base64," + base64.b64encode(blurred_bytes).decode('utf-8')
