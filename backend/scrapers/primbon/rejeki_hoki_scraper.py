...
def get_rejeki_hoki(payload):
    ...
    url = "https://www.primbon.com/rejeki_hoki_weton.php"
    post_data = {
        "tgl": str(tgl),
        "bln": str(bln),
        "thn": str(thn),
    ...
    try:
                ...
                if "Hari Lahir:" in txt:
                    if idx + 1 < len(body_div.contents):
                        ...
                        for j in range(idx + 2, len(body_div.contents)):
                            ...
                            if sub_node.name == "b" or sub_node.name == "a":
                                break
        ...
        img = body_div.find("img", src=lambda s: s and s.startswith("rejeki/"))
        if img:
            chart_image = "https://www.primbon.com/" + img["src"]

        if not hari_lahir_raw and not deskripsi:
            return {"success": False, "error": "Failed to parse hoki results."}
